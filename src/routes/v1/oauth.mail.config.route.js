...
router.route('/google').get(auth(), (req, res) => {
  ...
  res.send(
    `https://accounts.google.com/o/oauth2/v2/auth?client_id=${config.google.clientId}&redirect_uri=${config.domainScheme}://${config.domain}:${config.port}/v1/oauth-mail-configs/google/callback&response_type=code&scope=profile email https://mail.google.com/&include_granted_scopes=true&access_type=offline&state=${token}`
  );
});
